...
## Review and output-delivery corrections (September 9 follow-up)

Explicit output reads now return `payloadMode: "page"` with the page body. AgentAbject preserves this flag through direct and JobManager execution, retains the body, and displays the requested text verbatim up to 30,000 characters. ExternalCreator bounds requests to 30,000 bytes and continues to route reads through the task-owned RunningProcess. Small pages use the same path, avoiding a second truncation caused by JSON escaping. Unsolicited command output still uses previews. Loop signatures include the object ID, requested slice, and returned continuation coordinates, so reading different pages is progress while repeating an unchanged page still triggers steering.

Passing commands have no failure signatures; Node test-runner summaries supply their reported failure count. Expected errors logged by passing negative tests no longer appear as baseline failures. Nonzero exits and compiler/test failure evidence remain available for verification comparisons, including when no runner summary is recognized.

TaskReviewer now receives actual-result excerpts alongside predictions in its opening dossier, including legacy object-shaped observations. Its completion result can carry `assessments` and `applications` arrays, which the reviewer validates and records through GoalManager and KnowledgeBase messages. A review can therefore assess a modest task in one model response. New reusable lessons remain optional and use the existing learning actions. An empty review with observed predictions receives at most one correction to supply assessments; explicit evidence gaps, rejected writes, and remaining omissions settle as partial. Batch work is bounded and stops on cancellation. Checkpointed correction state prevents repeated nudges after restoration, and recorded limitations survive repeated acceptance checks.

Regression coverage checks the actual model-facing conversation through both execution paths, byte-page bounds, escaped small pages, false and real loop detection, passing negative-test logs, receiver-owned batch learning and replay, correction limits across restoration, partial settlement, and cancellation between learning messages. Live review quality and call-count changes still require another Abject run.

## Runtime action evidence (September 9 follow-up)

`read_chunk`, `recall`, `remember`, `replan`, and intermediate progress actions now preserve their predictions before execution and record the resulting observations, just like domain actions and submitted jobs. Full requested chunk text is available through GoalManager's observation messages; compact prediction previews do not replace it or consume the retained-payload cache. Interrupted runtime calls retain unknown outcomes. This adds no model calls or mandatory prediction gate.

Invalid chunk handles and ranges, rejected memory writes, and jobs that did not complete produce failure observations. Empty searches remain successful lookups with no matches; whether that contradicts an expectation belongs to semantic review. Progress evidence records that an event was sent, without claiming delivery or task completion. Pattern matches retain KnowledgeBase's selection receipts, so an explicitly declared application can bind to the selected revision even after an edit. Retrieval alone never creates an application or usefulness credit.

Bus-driven regression scenarios cover a two-page diff, runtime memory and replan actions, progress events, expected failures, cancellation during a memory write, and automatic provenance across a concurrent pattern revision. The full suite passes 195 tests; typechecking and the server build also pass. Another live Abject run is needed to assess model behavior with the additional evidence.

## Knowledge reconciliation and commit reporting (September 9 follow-up)

AgentAbject retains the bounded claim text actually injected, together with its source (profile, relevant knowledge, or pattern). TaskReviewer prioritizes relevant claims and declared applications over always-injected profile entries, presents historical and current claim excerpts separately, and includes completion evidence such as background test results. Per-entry limits keep large memories from consuming the whole knowledge budget. Full records and entries remain available through messages.

Reviewer completion accepts optional `knowledgeUpdates` containing evidence-backed `update_entry` or `archive_entry` actions. These use the existing KnowledgeBase handlers and user-authorship protections; acknowledged corrections survive review restoration without being replayed. Rejected corrections remain visible as partial learning. Cancellation during an ownership lookup prevents the following mutation. The reviewer is directed to repair both obsolete titles and content, preserve unrelated valid material, and archive obsolete duplicates rather than add another competing correction. Missing evidence remains uncertain, and retrieval alone earns no pattern credit. Existing workspace knowledge is corrected through reviewer messages; no database migration rewrites historical claims.

Chat and ScrumMaster distinguish recalled implementation suggestions from explicit user requirements. ExternalCreator groups predictable Git bookkeeping, and the runtime's response-format reminder now respects agents that allow action batching. Exact declared checks requested through `bash` share `verify` evidence reuse, including fresh-run overrides, failed-run invalidation, task identity, and current input snapshots. Arbitrary shell aliases and compound commands are not assumed equivalent to a declared check.

Completion reports include background checks and available test totals even when the task only reviewed and committed pre-existing changes. They distinguish source edits made during the task from repository activity, and mark historical or incomplete verification coverage explicitly. Reports use the current project snapshot without rerunning successful checks just to compose a summary.

Regression coverage includes stale claim correction and future recall, protected user entries, restoration and cancellation, commit-only verification reports, changed inputs, and forced or failed reruns. Model-driven correction quality and actual call savings require another live Abject run.
