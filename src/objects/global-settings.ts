...
export class GlobalSettings extends Abject {
  ...
  // Remote UI access widgets
  private remoteUIAccessId?: AbjectId;
  private remoteUIEnableCheckboxId?: AbjectId;
  private remoteUIStatusLabelId?: AbjectId;
  private remoteUIGenerateBtnId?: AbjectId;
  private remoteUIQrImageId?: AbjectId;
  private remoteUIQrUrlLabelId?: AbjectId;
  private remoteUIClientsLayoutId?: AbjectId;
  private remoteUIClientsHeaderId?: AbjectId;
  /** Map row-widget revoke buttons → peerId so we know which client to revoke. */
  private remoteUIRevokeBtns: Map<AbjectId, string> = new Map();
  /** Track row layouts so we can clear and rebuild the list. */
  private remoteUIRowIds: AbjectId[] = [];
...
- This object manages UI only; use it to show/hide the configuration window.`;
  ...
  protected override async onInit(): Promise<void> {
    ...
    if (this.remoteUIAccessId) {
      // Subscribe to clientsChanged so the auth tab refreshes when a phone
      // pairs, reconnects, or is revoked.
      try {
        await this.request(request(this.id, this.remoteUIAccessId, 'addDependent', {}));
      } catch { /* best-effort */ }
    }
  ...
  private setupHandlers(): void {
    ...
    this.on('changed', async (m: AbjectMessage) => {
      ...
      // Remote UI access — toggle enable
      if (fromId === this.remoteUIEnableCheckboxId && aspect === 'change') {
        if (this.remoteUIAccessId) {
          try {
            await this.request(request(this.id, this.remoteUIAccessId,
              'setEnabled', { enabled: !!value }));
          } catch (err) {
            log.warn(`setEnabled failed: ${err}`);
          }
          await this.refreshRemoteUIStatusLabel();
        }
        return;
      }

      // Remote UI access — Generate Pairing QR
      if (fromId === this.remoteUIGenerateBtnId && aspect === 'click') {
        if (this.remoteUIAccessId) {
          try {
            const result = await this.request<{ qrUrl: string; qrDataUrl: string }>(
              request(this.id, this.remoteUIAccessId, 'generatePairingToken', {})
            );
            if (this.remoteUIQrImageId) {
              await this.request(request(this.id, this.remoteUIQrImageId, 'update',
                { url: result.qrDataUrl, alt: '' }));
            }
            if (this.remoteUIQrUrlLabelId) {
              await this.request(request(this.id, this.remoteUIQrUrlLabelId, 'update',
                { text: result.qrUrl }));
            }
          } catch (err) {
            log.warn(`generatePairingToken failed: ${err}`);
          }
        }
        return;
      }

      // Remote UI access — Revoke a paired device
      if (this.remoteUIRevokeBtns.has(fromId) && aspect === 'click') {
        const peerId = this.remoteUIRevokeBtns.get(fromId);
        if (peerId && this.remoteUIAccessId) {
          try {
            await this.request(request(this.id, this.remoteUIAccessId,
              'revokeClient', { peerId }));
          } catch (err) {
            log.warn(`revokeClient failed: ${err}`);
          }
          // List will refresh via clientsChanged event
        }
        return;
      }

      // Remote UI access — clients changed (paired, reconnected, revoked)
      if (fromId === this.remoteUIAccessId && aspect === 'clientsChanged') {
        await this.refreshRemoteUIClientsList();
        await this.refreshRemoteUIStatusLabel();
        return;
      }
  ...
  private async buildAuthTab(): Promise<void> {
    ...
    // Lazy-discover in case onInit ran before RemoteUIAccess was spawned.
    if (!this.remoteUIAccessId) {
      this.remoteUIAccessId = (await this.discoverDep('RemoteUIAccess')) ?? undefined;
      if (this.remoteUIAccessId) {
        try {
    ...
    if (this.remoteUIAccessId) {
      await this.buildRemoteAccessSection(cId);
    }
  }

  /**
   * Append a "Remote Access" section to the Auth tab. Lets the user enable
   * remote UI access, generate a QR for pairing, and revoke paired devices.
   */
  private async buildRemoteAccessSection(cId: AbjectId): Promise<void> {
    // Section header
    const { widgetIds: [headerId] } = await this.request<{ widgetIds: AbjectId[] }>(
      request(this.id, this.widgetManagerId!, 'create', { specs: [
        { type: 'label', windowId: this.windowId, text: 'Remote Access',
          style: { color: this.theme.textHeading, fontWeight: 'bold', fontSize: 15 } },
      ]})
    );
    await this.request(request(this.id, cId, 'addLayoutChild', {
    ...
    // Read current status from RemoteUIAccess
    const status = await this.request<{
      enabled: boolean; peerId: string; signalingUrl: string;
      deviceLabel: string; connectedCount: number; authorizedCount: number;
    ...
    this.remoteUIEnableCheckboxId = enableCheckboxId;
    await this.request(request(this.id, this.remoteUIEnableCheckboxId, 'addDependent', {}));
    await this.request(request(this.id, enableRowId, 'addLayoutChild', {
      widgetId: this.remoteUIEnableCheckboxId,
      sizePolicy: { horizontal: 'expanding' },
      preferredSize: { height: 28 },
    }));
    ...
    const { widgetIds: [statusLabelId] } = await this.request<{ widgetIds: AbjectId[] }>(
      request(this.id, this.widgetManagerId!, 'create', { specs: [
        { type: 'label', windowId: this.windowId,
          text: this.formatRemoteUIStatus(status),
          style: { color: this.theme.textTertiary, fontSize: 12 } },
      ]})
    );
    this.remoteUIStatusLabelId = statusLabelId;
    await this.request(request(this.id, cId, 'addLayoutChild', {
      widgetId: this.remoteUIStatusLabelId,
      sizePolicy: { vertical: 'fixed' },
      preferredSize: { height: 18 },
    }));
    ...
    this.remoteUIGenerateBtnId = generateBtnId;
    await this.request(request(this.id, this.remoteUIGenerateBtnId, 'addDependent', {}));
    await this.request(request(this.id, generateRowId, 'addLayoutChild', {
      widgetId: this.remoteUIGenerateBtnId,
      sizePolicy: { horizontal: 'fixed' },
      preferredSize: { width: 200, height: 36 },
    }));
    await this.request(request(this.id, generateRowId, 'addLayoutSpacer', {}));

    // QR image (empty until user clicks Generate)
    const { widgetIds: [qrImageId] } = await this.request<{ widgetIds: AbjectId[] }>(
      request(this.id, this.widgetManagerId!, 'create', { specs: [
        { type: 'image', windowId: this.windowId, alt: 'No QR generated yet',
          style: { background: this.theme.windowBg,
                   color: this.theme.textTertiary, fontSize: 12, radius: 4 } },
      ]})
    );
    this.remoteUIQrImageId = qrImageId;
    await this.request(request(this.id, cId, 'addLayoutChild', {
      widgetId: this.remoteUIQrImageId,
      sizePolicy: { vertical: 'fixed', horizontal: 'fixed' },
      preferredSize: { width: 280, height: 280 },
    }));

    // QR URL label (selectable so user can copy on desktop)
    ...
    this.remoteUIQrUrlLabelId = qrUrlLabelId;
    await this.request(request(this.id, cId, 'addLayoutChild', {
      widgetId: this.remoteUIQrUrlLabelId,
      sizePolicy: { vertical: 'fixed', horizontal: 'expanding' },
      preferredSize: { height: 32 },
    }));

    // Paired devices header
    const { widgetIds: [clientsHeaderId] } = await this.request<{ widgetIds: AbjectId[] }>(
      request(this.id, this.widgetManagerId!, 'create', { specs: [
        { type: 'label', windowId: this.windowId,
          text: this.formatPairedHeader(status.authorizedCount),
          style: { color: this.theme.textHeading, fontWeight: 'bold', fontSize: 13 } },
      ]})
    );
    this.remoteUIClientsHeaderId = clientsHeaderId;
    await this.request(request(this.id, cId, 'addLayoutChild', {
      widgetId: this.remoteUIClientsHeaderId,
      sizePolicy: { vertical: 'fixed' },
      preferredSize: { height: 22 },
    }));

    // Clients list container
    const clientsLayoutId = await this.request<AbjectId>(
    ...
    this.remoteUIClientsLayoutId = clientsLayoutId;
    await this.request(request(this.id, cId, 'addLayoutChild', {
      widgetId: this.remoteUIClientsLayoutId,
      sizePolicy: { vertical: 'fixed', horizontal: 'expanding' },
  ...
  private formatRemoteUIStatus(status: { enabled: boolean; peerId: string; signalingUrl: string; connectedCount: number }): string {
  ...
  private formatPairedHeader(count: number): string {
    return count === 0 ? 'Paired devices (none)' : `Paired devices (${count})`;
  }

  private async refreshRemoteUIClientsList(): Promise<void> {
    if (!this.remoteUIAccessId || !this.remoteUIClientsLayoutId) return;
    const clients = await this.request<Array<{ peerId: string; name: string; addedAt: number; lastConnected?: number }>>(
      request(this.id, this.remoteUIAccessId, 'listClients', {})
    );

    // Tear down existing rows
    for (const rowId of this.remoteUIRowIds) {
      try { await this.request(request(this.id, rowId, 'destroy', {})); } catch { /* ignore */ }
    }
    this.remoteUIRowIds = [];
    this.remoteUIRevokeBtns.clear();

    // Update header count
    if (this.remoteUIClientsHeaderId) {
      try {
        await this.request(request(this.id, this.remoteUIClientsHeaderId, 'update',
          { text: this.formatPairedHeader(clients.length) }));
      } catch { /* ignore */ }
    }

    // Resize layout to fit new rows
    const rowHeight = 32;
    const totalHeight = Math.max(0, clients.length * rowHeight);
    try {
      await this.request(request(this.id, this.remoteUIClientsLayoutId, 'setPreferredSize',
        { height: totalHeight }));
    } catch { /* ignore */ }
    ...
    for (const client of clients) {
      const rowId = await this.request<AbjectId>(
        request(this.id, this.widgetManagerId!, 'createNestedHBox', {
          parentLayoutId: this.remoteUIClientsLayoutId,
          margins: { top: 0, right: 0, bottom: 0, left: 0 },
          spacing: 8,
        })
      );
      this.remoteUIRowIds.push(rowId);
      await this.request(request(this.id, this.remoteUIClientsLayoutId, 'addLayoutChild', {
        widgetId: rowId,
        sizePolicy: { vertical: 'fixed', horizontal: 'expanding' },
        preferredSize: { height: rowHeight },
      }));

      const lastSeen = client.lastConnected
        ? new Date(client.lastConnected).toLocaleString()
        : 'never';
      const { widgetIds: [labelId, revokeBtnId] } = await this.request<{ widgetIds: AbjectId[] }>(
        request(this.id, this.widgetManagerId!, 'create', { specs: [
          { type: 'label', windowId: this.windowId,
            text: `${client.name || 'Remote'} — ${client.peerId.slice(0, 12)}… (last: ${lastSeen})`,
            style: { color: this.theme.textPrimary, fontSize: 12 } },
          { type: 'button', windowId: this.windowId, text: 'Revoke',
            style: { background: this.theme.destructiveBg,
      ...
      this.remoteUIRevokeBtns.set(revokeBtnId, client.peerId);
    }
  }

  private async refreshRemoteUIStatusLabel(): Promise<void> {
    if (!this.remoteUIAccessId || !this.remoteUIStatusLabelId) return;
    try {
      const status = await this.request<{
        enabled: boolean; peerId: string; signalingUrl: string;
        deviceLabel: string; connectedCount: number; authorizedCount: number;
      }>(request(this.id, this.remoteUIAccessId, 'getStatus', {}));
      await this.request(request(this.id, this.remoteUIStatusLabelId, 'update',
        { text: this.formatRemoteUIStatus(status) }));
    } catch { /* ignore */ }
  }
