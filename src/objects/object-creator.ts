...
When invited to a Sprint Plan, describe the concrete authoring or modification I'd perform, the target Abject (by name), and what would change. If the goal is purely runtime (open a window, call a method, send a Slack message), reply PASS.`;
  ...
  private draftSourceSystemPrompt(): string {
    return [
      ...
      'RUNTIME: handler code runs in a sandboxed backend (a Node vm inside a worker thread), NOT in a browser. There is NO `window`, `document`, `navigator`, `localStorage`, `fetch`, `WebSocket`, `AudioContext`/`webkitAudioContext`, `Image`, DOM, or `setTimeout`/`setInterval` as globals. The only ambient globals are `Math, JSON, Date, Array, Object, String, Number, Boolean, RegExp` plus your `this.*` helpers (`this.call`, `this.dep`, `this.data`, `this.changed`, `this.ensure`, `this.invariant`). Reaching for a browser API silently does nothing (or is caught and dropped), so the feature never runs — do not write it.',
      'EVERY host capability is another Abject reached by message, never a browser API. Discover it with `this.dep("Name")` then `this.call(id, method, payload)`. Map of intent → object: SOUND/tones/SFX/ambience → AudioOutput (`play` for files/URLs, `playTone` for beeps, `playGraph` for synthesized oscillator/noise graphs incl. looping drones); text-to-speech → Speech; HTTP → HttpClient; persistence → Storage; files → FileSystem; scheduling/delays → Timer or Scheduler (never `setTimeout`); clipboard → Clipboard; drawing → the canvas widget via `draw` commands. If you are unsure which object provides something, `call("Registry", "ask", { question: "what object plays sound?" })` before assuming an API exists.',
...
Anything the browser would give you is instead a first-class Abject you reach by message. Discover with \`describe\`/\`ask\`/\`Registry.discover\`, then \`call(id, method, payload)\`:
- **Sound** (tones, SFX, ambient loops, chimes) → **AudioOutput**: \`play\` (file/URL/data:/abject:// source), \`playTone\` (a single beep), \`playGraph\` (synthesized oscillator + noise voices with envelope/filter/glide; \`loop:true\` for sustained drones). Never build \`AudioContext\`/oscillators in source — synthesize declaratively through \`playGraph\`.
- **Speech** (text-to-speech / recognition) → Speech. **HTTP** → HttpClient. **Persistence** → Storage. **Files** → FileSystem. **Delays/intervals** → Timer or Scheduler. **Clipboard** → Clipboard. **Drawing** → the canvas widget via \`draw\` commands (never a 2D context directly).

When a goal mentions sound, music, or audio and you are drafting or modifying source, wire it through AudioOutput. If you are unsure an object exists for some capability, \`call("Registry", "ask", { question: "..." })\` rather than assuming a browser API.
