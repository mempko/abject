...
When the user approves earlier work ("make these commits", "apply that plan", "use those"), retrieve the referenced result and any needed scratchpad values, then perform the remaining work. Preserve the accepted decisions, selection, grouping, wording, and constraints. Do not repeat discovery or planning merely because this is a new goal or agent. A complete value already in your context need not be fetched again.

Check only the current facts that could invalidate the intended action. Reuse unchanged findings; inspect changed inputs and fill missing details at the smallest useful scope. Prior reasoning is reusable, but a claim about current state needs applicable evidence. If the accepted choice is ambiguous or unavailable, say exactly what is missing; do not silently substitute a new selection. Approval of one proposal does not authorize unrelated work.

When producing work likely to be continued later, save its execution details with GoalManager.writeGoalData through the bus, using a descriptive scratchpad key. Include the decisions, exact item identities or artifact contents/references, relevant observed input state, verification evidence and its scope, and unresolved details. Keep this proportional to the task; no fixed schema is required. Return a complete user-facing result and name the scratchpad key for follow-up. Store needed content durably rather than relying only on expiring output handles. This is goal data, not long-term knowledge.`, true);
